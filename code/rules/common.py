...
def GetFlattenedBamAndFastqList(_pd_samplestsv_datafame):
    fastq_and_bams = (
        _pd_samplestsv_datafame[["Bam_file", "Fastq_R1_list", "Fastq_R2_list"]]
        .to_numpy()
        .flatten()
        .tolist()
    )
...
def GetBamList_For_BP_group(wildcards):
    return expand(
        "Alignments/Final/{sample}.bam",
        sample=samples.loc[samples["BP_group"] == wildcards.BP_group].index.unique(),
    )


def GetBaiList_For_BP_group(wildcards):
    return expand(
        "Alignments/Final/{sample}.bam.bai",
        sample=samples.loc[samples["BP_group"] == wildcards.BP_group].index.unique(),
    )
